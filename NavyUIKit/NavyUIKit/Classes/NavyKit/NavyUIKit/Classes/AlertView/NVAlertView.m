...
- (instancetype) init {
    ...
    if (self) {
        ...
        self.uiTitle.textColor          = COLOR_HM_TEXT;
    }
...
- (instancetype) initWithTitle:(NSString *)title
    ...
    if (self) {
        ...
        contentView.textColor           = COLOR_HM_TEXT;
...
- (void) updateActionsDisplay {
    ...
    [self.arrayActions enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([action.title isKindOfClass:[NSAttributedString class]]) {
            ...
            [button setTitleColor:COLOR_HM_THEME_SUB forState:UIControlStateNormal];
...
+ (void) showAlterWithTitle:(NSString *)title content:(NSString *)content {
    NVAlertController *ac = [[NVAlertController alloc] initWithTitle:title content:content];
    [ac show];
}

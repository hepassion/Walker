...
- (instancetype)initTitle:(NSString *) title color:(UIColor *) color delay:(CGFloat) delay
{
    self = [super init];
    if (self) {
        _buttonTitle = title;
...
- (void) restore {
    ...
    BOOL isActiveLoading = self.uiLoadingView.frame.origin.y >= 0;
    BOOL isActiveResult = self.uiResultView.frame.origin.y >= 0;
    ...
    [UIView animateWithDuration:0.35f
                     animations:^{
                         CGRect frame = CGRectMake(0.0f, -size.height, size.width, size.height);
                         if (isActiveResult) {
                             self.uiResultView.frame = frame;
                         }

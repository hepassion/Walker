...
- (void) nv_showExceptionViewInView:(UIView*)view {
    ...
    [self bringPageNavigationBarToFront];
}
...
- (void)bringPageNavigationBarToFront {
    SEL selector = NSSelectorFromString(@"getCustomNavigationBar");
    BOOL havePageNavi = [self performSelector:selector];
    if (havePageNavi) {
        [self.view bringSubviewToFront:[self performSelector:@selector(pageNavigationBar)]];
    }
}

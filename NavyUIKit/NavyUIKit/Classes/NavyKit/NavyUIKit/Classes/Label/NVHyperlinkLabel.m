...
- (void) addHyperlink:(NSString *)hyperlink withUrl:(NSString*)urlPath {
    ...
    NSAttributedString* attributed = [[NSAttributedString alloc] initWithString:hyperlink
                                                                     attributes:ATTR_DICTIONARY(COLOR_HM_THEME, 14.0f)];
...
- (void)touchesEnded:(NSSet *)touches
    ...
    if ((self.delegate && [self.delegate respondsToSelector:@selector(hyperlinkLabel:touchUrl:)]) || self.touchUrlBlock) {
        ...
        [self.array enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if (point.x >= range.location + delta
                ...
                NSString *url = [dictionary nvObjectForKey:@"url"];
                if (url && url.length) {
                    [self.delegate hyperlinkLabel:self touchUrl:url];
                    BLOCK_SAFE_RUN(self.touchUrlBlock,url);
                }
            }
        }];
    }

...
- (void) reloadDataWithTitles:(NSArray<NSString *> *(^)(void))titleOfItems {
    ...
    [self reloadDataWithTitles:titleOfItems
              itemViewInstance:^NVMenuItemView *(NSUInteger index) {
                  NVMenuItemView *itemView = [[NVMenuItemView alloc] init];
                  return itemView;
              }];
    
}

- (void) reloadDataWithTitles:(NSArray<NSString *> *(^)(void))titleOfItems itemViewInstance:(NVMenuItemView *(^)(NSUInteger))viewOfItem {
    ...
    if ([arrayTitles isKindOfClass:[NSArray class]]) {
        ...
        [arrayTitles enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if (itemView.normalColor) {
                [itemView setTitleColor:itemView.normalColor forState:UIControlStateNormal];
            }
            
            if (itemView.selectedColor) {
                [itemView setTitleColor:itemView.selectedColor forState:UIControlStateSelected];
            }
            
            if (itemView.normalBackgroundColor) {
                [itemView setBackgroundImage:[UIImage jk_imageWithColor:itemView.normalBackgroundColor]
                                    forState:UIControlStateNormal];
            }
            
            if (itemView.disableColor) {
                [itemView setTitleColor:itemView.disableColor forState:UIControlStateDisabled];
            }
            
            if (itemView.selectedBackgroundColor) {
                [itemView setBackgroundImage:[UIImage jk_imageWithColor:itemView.selectedBackgroundColor]
...
-(void)setUnableIndex:(NSInteger)unableIndex{
    ...
    if (unableIndex < 0) {
        ...
        NVMenuItemView* item = (NVMenuItemView*)[self.arrayButtonItems objectAtIndex:unableIndex];
        item.enabled = NO;
    }

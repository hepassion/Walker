...
- (id) initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        _uiNoticeLabel = [[NVLabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - FIT6(124) - FIT6(10),
                                                                   (CELL_HEIGHT - 20.0f)/2,
                                                                   FIT6(124),
                                                                   20.0f)];
        [self.contentView addSubview:_uiNoticeLabel];
        [_uiNoticeLabel setTextColor:[UIColor grayColor]];
        [_uiNoticeLabel setFont:nvNormalFontWithSize(14.0f + fontScale)];
        _uiNoticeLabel.hidden = YES;
    }
...
//    _uiTitle.frame = CGRectMake(15.0f, (self.contentView.bounds.size.height - 20.0f)/2, 260.0f, 20.0f);
    ...
    CGSize titleSize = [_uiTitle sizeThatFits:CGSizeMake(0, _uiTitle.height)];
    CGFloat titleWidth = titleSize.width > FIT6(160) ? titleSize.width : FIT6(160);
    _uiTitle.frame = CGRectMake(15.0f, (self.contentView.bounds.size.height - 20.0f)/2,titleWidth, 20.0f);
    ...
    switch (self.item.cellStyle) {
        ...
        case NVSheetTableViewCellStyleEdit4:
        {
            [self setSelectionStyle:UITableViewCellSelectionStyleNone];
            
            _uiTitle.hidden = (self.item.cellStyle == NVSheetTableViewCellStyleEdit2);
            CGFloat x = (self.item.cellStyle == NVSheetTableViewCellStyleEdit2) ? 30.0f : _uiTitle.right + FIT6(15);
            ...
            if (field == nil) {
                field = [[NVTextField alloc] initWithFrame:CGRectMake(x,
                                                                      (dataModel.cellHeight.floatValue - 30.0f)/2,
            ...
            if ([dataModel.value isKindOfClass:[NSAttributedString class]]) {
                field.attributedText= dataModel.value;
            } else {
            ...
            if (self.item.cellStyle == NVSheetTableViewCellStyleEdit4) {
                _uiNoticeLabel.hidden = NO;
                field.width = FIT6(350);
                [_uiNoticeLabel setTextColor:self.item.noticeTextColor];
                _uiNoticeLabel.text = self.item.noticeText;
            } else {
                _uiNoticeLabel.hidden = YES;
            }
        ...
        case NVSheetTableViewCellStyleEdit3:
        {
            ...
            if (rightButton == nil) {
                rightButton = [[NVButton alloc] initWithFrame:CGRectMake(field.frame.origin.x + field.frame.size.width,
                                                                         ...
                                                                         30.0f,
                                                                         30.0f)];
                rightButton.tag = TAG_SHEET_CELL_RIGHTBUTTON;
        ...
        case NVSheetTableViewCellStyleEditButton:
        {
            ...
            NVTextField* field = (NVTextField*)[self.contentView viewWithTag:TAG_SHEET_CELL_FIELD];
            if (field) {
                field = [[NVTextField alloc] initWithFrame:CGRectMake(80.0f, (dataModel.cellHeight.floatValue - 30.0f)/2, 230.0f, 30.0f)];
                [self.contentView addSubview:field];
                field.tag = TAG_SHEET_CELL_FIELD;
            }
            ...
            if (button) {
                button = [[UIButton alloc] initWithFrame:CGRectMake(80.0f, (dataModel.cellHeight.floatValue - 30.0f)/2, 230.0f, 30.0f)];
    ...
    if (dataModel.controlsVerticalAlignCenter) {
        _uiTitle.centerY = dataModel.cellHeight.doubleValue/2;
        UITextField *field = [self viewWithTag:TAG_SHEET_CELL_FIELD];
        field.centerY = dataModel.cellHeight.doubleValue/2;
        
        UIView *view = [self.contentView viewWithTag:TAG_SHEET_CELL_RIGHTBUTTON];
        if(view){
            view.centerY = dataModel.cellHeight.floatValue / 2;
        }
    }
}
...
- (id) initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        _uiValue.tag            = TAG_SHEET_CELL_VALUE_LABEL;
...
- (void) setObject:(id)object {
    ...
    CGFloat  cellHeight             = dataModel.cellHeight.floatValue;
    self.introView.left             =  APPLICATIONWIDTH - self.introView.width - 10.0f * displayScale;
    self.introView.top              = (cellHeight - self.introView.height)/2;
    
    _bgLayer.frame                  = CGRectMake(0.0f, 0.0f, APPLICATIONWIDTH, cellHeight);
    
    _uiValue.height                 = cellHeight;
    
    if (self.introView.hidden) {
        //如果隐藏箭头 调整value的位置
        _uiValue.right = APPLICATIONWIDTH - FIT6(30);
    }
